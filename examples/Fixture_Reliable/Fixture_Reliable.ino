/*
  RF24StageLink - Fixture_Reliable   (POINT-TO-POINT / ONE fixture)

  Same as Fixture_Receive, but RELIABLE mode: it auto-ACKs every MIDI message so the
  master can retry until it gets through (guaranteed delivery) and knows it arrived.
  Pairs with Master_UsbMidi_Reliable. Prints the decoded MIDI via TYPED callbacks.

  *** ONE FIXTURE ONLY ***
  Reliable mode is point-to-point. Do NOT run more than one fixture in reliable mode
  on the same address - their ACKs collide and the link stalls. For a multi-fixture
  rig use the plain Fixture_Receive (broadcast) instead.
  setReliable(true) MUST match the master (Master_UsbMidi_Reliable).

  Works on any board. Uses the default channel list + address, so it pairs with an
  unmodified Master_UsbMidi_Reliable out of the box.
...
// Optional custom hop set / address (must match the master).
...
void setup() {
  ...
  // ---- optional RF24StageLink config: BEFORE beginFixture(); must match the master ----
  link.setReliable(true);                        // <-- ACK back to the master (ONE fixture; set on BOTH ends)
